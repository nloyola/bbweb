...
object Centre {
  ...
  val sort2Compare: Map[String, CentresCompare] =
    Map[String, CentresCompare]("name" -> Centre.compareByName, "state" -> Centre.compareByState)
 ...
 */
final case class DisabledCentre(
    id:           CentreId,
    version:      Long,
  ...
  def withStudyId(studyId: StudyId): DomainValidation[DisabledCentre] =
    validateId(studyId, InvalidStudyId).map { _ =>
      copy(studyIds = studyIds + studyId, version = version + 1, timeModified = Some(OffsetDateTime.now))
  ...
  def withLocation(location: Location): DomainValidation[DisabledCentre] =
    checkAddLocation(location).map { _ =>
      ...
      copy(locations = newLocations, version = version + 1, timeModified = Some(OffsetDateTime.now))
    }
  ...
  protected def checkRemoveLocation(locationId: LocationId): DomainValidation[Location] =
    locations
      .find { x =>
        x.id == locationId
      }
  ...
  protected def nameNotUsed(location: Location): DomainValidation[Unit] = {
    ...
    locations
      .find { x =>
        (x.name.toLowerCase == nameLowerCase) && (x.id != location.id)
      } match {
      case Some(_) =>
...
object DisabledCentre extends CentreValidations {
  ...
  def create(
      ...
      studyIds:    Set[StudyId],
      locations:   Set[Location]
    ): DomainValidation[DisabledCentre] = {
 ...
 */
final case class EnabledCentre(
    id:           CentreId,
    version:      Long,
    timeAdded:    OffsetDateTime,
    timeModified: Option[OffsetDateTime],
