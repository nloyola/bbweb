...
class StudiesProcessor @Inject()(
  ...
  val receiveRecover: Receive = {
    case event: StudyEvent =>
      event.eventType match {
        ...
        case et: StudyEvent.EventType.AnnotationTypeUpdated =>
          applyParticipantAnnotationTypeUpdatedEvent(event)
        case et: StudyEvent.EventType.AnnotationTypeRemoved =>
  ...
  private def addCmdToEvent(cmd: AddStudyCmd): ServiceValidation[StudyEvent] =
    for {
      ...
      StudyEvent(study.id.id).update(_.optionalSessionUserId := cmd.sessionUserId,
  ...
  private def addAnnotationTypeCmdToEvent(
      cmd:   StudyAddParticipantAnnotationTypeCmd,
      study: DisabledStudy
    ): ServiceValidation[StudyEvent] =
    for {
      annotationType <- {
    ...
    } yield StudyEvent(study.id.id).update(_.optionalSessionUserId := cmd.sessionUserId,
                                           _.time := OffsetDateTime.now
                                             .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
  ...
  private def updateAnnotationTypeCmdToEvent(
      cmd:   StudyUpdateParticipantAnnotationTypeCmd,
      study: DisabledStudy
    ): ServiceValidation[StudyEvent] =
    for {
  ...
  private def removeAnnotationTypeCmdToEvent(
      cmd:   UpdateStudyRemoveAnnotationTypeCmd,
      study: DisabledStudy
    ): ServiceValidation[StudyEvent] =
  ...
  private def enableCmdToEvent(cmd: EnableStudyCmd, study: DisabledStudy): ServiceValidation[StudyEvent] = {
    ...
    } else if (collectionEventTypes.filter { cet =>
                 cet.hasSpecimenDefinitions
               }.isEmpty) {
  ...
  private def disableCmdToEvent(cmd: DisableStudyCmd, study: EnabledStudy): ServiceValidation[StudyEvent] =
    study.disable map { _ =>
      StudyEvent(study.id.id).update(_.optionalSessionUserId := cmd.sessionUserId,
                                     _.time := OffsetDateTime.now
                                       .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
  ...
  private def retireCmdToEvent(cmd: RetireStudyCmd, study: DisabledStudy): ServiceValidation[StudyEvent] =
    study.retire map { _ =>
      StudyEvent(study.id.id).update(_.optionalSessionUserId := cmd.sessionUserId,
                                     _.time := OffsetDateTime.now
                                       .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
  ...
  private def unretireCmdToEvent(cmd: UnretireStudyCmd, study: RetiredStudy): ServiceValidation[StudyEvent] =
  ...
  private def onValidEventStudyAndVersion(
      event:        StudyEvent,
      eventType:    Boolean,
      eventVersion: Long
  ...
  private def onValidEventEnabledStudyAndVersion(
      event:        StudyEvent,
      eventType:    Boolean,
      eventVersion: Long
  ...
  private def onValidEventRetiredStudyAndVersion(
      event:        StudyEvent,
      eventType:    Boolean,
      eventVersion: Long
  ...
  private def putDisabledOnSuccess(
      validation: ServiceValidation[DisabledStudy],
      eventTime:  OffsetDateTime
    ): ServiceValidation[Unit] = {
    validation.foreach { s =>
      studyRepository.put(s.copy(timeModified = Some(eventTime)))
    }
    validation.map { _ =>
      ()
    }
  ...
  private def putEnabledOnSuccess(
      validation: ServiceValidation[EnabledStudy],
      eventTime:  OffsetDateTime
    ): ServiceValidation[Unit] = {
    validation.foreach { s =>
      studyRepository.put(s.copy(timeModified = Some(eventTime)))
  ...
  private def putRetiredOnSuccess(
      validation: ServiceValidation[RetiredStudy],
      eventTime:  OffsetDateTime
    ): ServiceValidation[Unit] = {
    validation.foreach { s =>
      studyRepository.put(s.copy(timeModified = Some(eventTime)))
  ...
  private def applyNameUpdatedEvent(event: StudyEvent): Unit =
    onValidEventDisabledStudyAndVersion(event, event.eventType.isNameUpdated, event.getNameUpdated.getVersion) {
      (study, eventTime) =>
  ...
  private def applyEnabledEvent(event: StudyEvent): Unit =
    onValidEventDisabledStudyAndVersion(event, event.eventType.isEnabled, event.getEnabled.getVersion) {
      (study, eventTime) =>
        putEnabledOnSuccess(study.enable, eventTime)
    }

  private def applyDisabledEvent(event: StudyEvent): Unit =
    onValidEventEnabledStudyAndVersion(event, event.eventType.isDisabled, event.getDisabled.getVersion) {
      (study, eventTime) =>
        putDisabledOnSuccess(study.disable, eventTime)
    }

  private def applyRetiredEvent(event: StudyEvent): Unit =
    onValidEventDisabledStudyAndVersion(event, event.eventType.isRetired, event.getRetired.getVersion) {
      (study, eventTime) =>
        putRetiredOnSuccess(study.retire, eventTime)
    }

  private def applyUnretiredEvent(event: StudyEvent): Unit =
    onValidEventRetiredStudyAndVersion(event, event.eventType.isUnretired, event.getUnretired.getVersion) {
      (study, eventTime) =>
