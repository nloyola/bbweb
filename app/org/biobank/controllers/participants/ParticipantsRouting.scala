...
import org.biobank.domain.participants.{CollectionEventId, ParticipantId, SpecimenId}
import play.api.mvc.PathBindable.Parsing
import play.api.routing.sird._

object ParticipantsRouting {

  implicit object bindableParticipantId
      extends Parsing[ParticipantId](ParticipantId.apply,
                                     _.id,
                                     (key: String, e: Exception) => s"$key is not a valid participant Id")

  implicit object bindableCollectionEventId
      extends Parsing[CollectionEventId](CollectionEventId.apply,
                                         _.id,
                                         (key: String, e: Exception) =>
                                           s"$key is not a valid collection event Id")

  implicit object bindableSpecimenId
      extends Parsing[SpecimenId](SpecimenId.apply,
                                  _.id,
