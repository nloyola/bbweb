  ...
  class GroupMatcher extends Matcher[Expression] {
    ...
    def apply(left: Expression) =
      MatchResult(left match {
        case Group(_) => true
        case _        => false
      }, s"""Expression $left is not a Group"""", s"""Expression $left is a Group"""")
  }

  def beComparison(expectedSelector: String, expectedComparator: Comparator, expectedArgument: String) =
    ...
    it("must parse an AND expression within an OR expresssion") {
      ...
      inside(result.value) {
        case OrExpression(expressions) =>
          expressions must have length (2)
          expressions.zipWithIndex.foreach {
            case (andExpression, orIndex) =>
              inside(andExpression) {
                case AndExpression(expressions) =>
                  expressions must have length (2)
                  expressions.zipWithIndex.foreach {
                    case (comparison, andIndex) =>
    ...
    it("must parse an OR expression within an AND expresssion") {
      ...
      inside(result.value) {
        case AndExpression(expressions) =>
          expressions must have length (2)
          expressions.zipWithIndex.foreach {
            case (orExpression, andIndex) =>
              inside(orExpression) {
                case OrExpression(expressions) =>
                  expressions must have length (2)
                  expressions.zipWithIndex.foreach {
                    case (comparison, orIndex) =>
