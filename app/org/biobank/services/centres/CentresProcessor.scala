...
class CentresProcessor @Inject()(
  ...
  private def addCentreCmdToEvent(cmd: AddCentreCmd): ServiceValidation[CentreEvent] =
    for {
      ...
      newCentre <- DisabledCentre.create(id = centreId,
                                         ...
                                         studyIds    = Set.empty,
                                         locations   = Set.empty)
    } yield CentreEvent(newCentre.id.id).update(_.sessionUserId := cmd.sessionUserId,
                                                _.time := OffsetDateTime.now
                                                  .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                                                _.added.name                := cmd.name,
                                                _.added.optionalDescription := cmd.description)

  private def updateCentreNameCmdToEvent(
      cmd:    UpdateCentreNameCmd,
      centre: DisabledCentre
    ): ServiceValidation[CentreEvent] =
    for {
      nameAvailable <- nameAvailable(cmd.name, centre.id)
      centre        <- centre.withName(cmd.name)
    } yield CentreEvent(centre.id.id).update(_.sessionUserId := cmd.sessionUserId,
                                             _.time := OffsetDateTime.now
                                               .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                                             _.nameUpdated.version := cmd.expectedVersion,
                                             _.nameUpdated.name    := cmd.name)

  private def updateCentreDescriptionCmdToEvent(
      cmd:    UpdateCentreDescriptionCmd,
      centre: DisabledCentre
    ): ServiceValidation[CentreEvent] =
    centre.withDescription(cmd.description).map { _ =>
      CentreEvent(centre.id.id).update(_.sessionUserId := cmd.sessionUserId,
                                       _.time := OffsetDateTime.now
                                         .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
  ...
  private def enableCentreCmdToEvent(
      cmd:    EnableCentreCmd,
      centre: DisabledCentre
    ): ServiceValidation[CentreEvent] =
    centre.enable.map { _ =>
  ...
  private def disableCentreCmdToEvent(
      cmd:    DisableCentreCmd,
   ...
   */
  private def addLocationCmdToEvent(
      cmd:    AddCentreLocationCmd,
      centre: DisabledCentre
    ): ServiceValidation[CentreEvent] =
    for {
      location <- {
   ...
   * FIXME: Locations should be added regardless of the centre's status.
   */
  private def updateLocationCmdToEvent(
      cmd:    UpdateCentreLocationCmd,
      centre: DisabledCentre
    ): ServiceValidation[CentreEvent] =
    for {
      location <- {
   ...
   */
  private def removeLocationCmdToEvent(
      cmd:    RemoveCentreLocationCmd,
      centre: DisabledCentre
    ): ServiceValidation[CentreEvent] =
  ...
  private def onValidEventCentreAndVersion(
      event:        CentreEvent,
      eventType:    Boolean,
      eventVersion: Long
    )(applyEvent:   (Centre, CentreEvent, OffsetDateTime) => ServiceValidation[Centre]
    ): Unit =
    if (!eventType) {
  ...
  private def onValidEventDisabledCentreAndVersion(
      ...
      eventType:    Boolean,
      eventVersion: Long
    )(applyEvent:   (DisabledCentre, CentreEvent, OffsetDateTime) => ServiceValidation[Centre]
    ): Unit =
  ...
  private def onValidEventEnabledCentreAndVersion(
      event:        CentreEvent,
      eventType:    Boolean,
      eventVersion: Long
    )(applyEvent:   (EnabledCentre, CentreEvent, OffsetDateTime) => ServiceValidation[Centre]
    ): Unit =
    onValidEventCentreAndVersion(event, eventType, eventVersion) { (centre, event, eventTime) =>
      centre match {
  ...
  private def applyAddedEvent(event: CentreEvent): Unit =
    if (!event.eventType.isAdded) {
      ...
      val validation = DisabledCentre
        .create(id          = CentreId(event.id),
                version     = 0L,
  ...
  private def applyNameUpdatedEvent(event: CentreEvent): Unit =
    onValidEventDisabledCentreAndVersion(event,
      ...
      val v = centre.withName(event.getNameUpdated.getName).map { c =>
        c.copy(slug = centreRepository.uniqueSlugFromStr(c.name), timeModified = Some(eventTime))
      }
  ...
  private def applyDescriptionUpdatedEvent(event: CentreEvent): Unit =
    onValidEventDisabledCentreAndVersion(event,
      ...
      v.foreach { c =>
        centreRepository.put(c.copy(timeModified = Some(eventTime)))
      }
      v
    }

  private def applyEnabledEvent(event: CentreEvent): Unit =
    onValidEventDisabledCentreAndVersion(event, event.eventType.isEnabled, event.getEnabled.getVersion) {
      (centre, _, eventTime) =>
        val v = centre.enable
        v.foreach { c =>
          centreRepository.put(c.copy(timeModified = Some(eventTime)))
  ...
  private def applyDisabledEvent(event: CentreEvent): Unit =
    onValidEventEnabledCentreAndVersion(event, event.eventType.isDisabled, event.getDisabled.getVersion) {
      (centre, _, eventTime) =>
  ...
  private def applyStudyAddedEvent(event: CentreEvent): Unit =
    onValidEventDisabledCentreAndVersion(event, event.eventType.isStudyAdded, event.getStudyAdded.getVersion) {
      (centre, _, eventTime) =>
