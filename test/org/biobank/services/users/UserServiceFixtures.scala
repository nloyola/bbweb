...
trait UserServiceFixtures {
  ...
  class UsersFixture(adminUser: ActiveUser, val nonAdminUser: ActiveUser, val user: ActiveUser)
      extends AdminUserFixture(adminUser)

  class UsersFixtureWithPassword(
  ...
  protected def restoreRoles(): Unit = {
    accessItemRepository.getValues
      ...
      .filter { r =>
        r.id != RoleId.WebsiteAdministrator
      }
