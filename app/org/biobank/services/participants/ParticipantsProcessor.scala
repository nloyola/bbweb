...
class ParticipantsProcessor @Inject()(
  ...
  private def updateUniqueIdCmdToEvent(
    ...
    ): ServiceValidation[ParticipantEvent] =
    for {
  ...
  private def removeAnnotationCmdToEvent(
      cmd:         ParticipantRemoveAnnotationCmd,
      study:       Study,
      participant: Participant
    ): ServiceValidation[ParticipantEvent] =
    for {
  ...
  private def processUpdateCmd[T <: ParticipantModifyCommand](
      ...
      applyEvent: ParticipantEvent => Unit
    ): Unit = {
  ...
  private def applyAddedEvent(event: ParticipantEvent) =
    if (!event.eventType.isAdded) {
      ...
      val v = Participant
        .create(studyId     = StudyId(addedEvent.getStudyId),
  ...
  private def onValidEventAndVersion(
    ...
    if (!eventType) {
      ...
      participantRepository
        .getByKey(ParticipantId(event.id)).fold(
          err => log.error(s"participant from event does not exist: $err"),
          participant => {
            if (participant.version != eventVersion) {
              log.error(
                s"event version check failed: participant version: ${participant.version}, event: $event"
              )
            } else {
  ...
  private def applyUniqueIdUpdatedEvent(event: ParticipantEvent) =
    onValidEventAndVersion(event, event.eventType.isUniqueIdUpdated, event.getUniqueIdUpdated.getVersion) {
   ...
   */
  protected def uniqueIdAvailableMatcher(
      uniqueId: String
    )(matcher:  Participant => Boolean
