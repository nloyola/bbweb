...
class UsersController @Inject()(
  ...
  def login(): Action[JsValue] =
    Action.async(parse.json) { implicit request =>
      ...
      val v = for {
        ...
        for {
          ...
          result <- silhouette.env.authenticatorService
                     .embed(token, Ok(Json.toJson(Token(user, token, authenticator.expirationDateTime))))
      ...
      v.fold(err => Future.successful(play.api.mvc.Results.Unauthorized), result => result)
    }
  ...
  def authenticateUser(): Action[AnyContent] = silhouette.SecuredAction.async { implicit request =>
    Future {
      ...
      v.fold(err => Unauthorized, user => Ok(user))
    }
  }
  ...
  def snapshot: Action[Unit] =
    action(parse.empty) { implicit request =>
      validationReply(usersService.snapshotRequest(request.identity.user.id).map { _ =>
        true
      })
    }
  ...
  def update(id: UserId): Action[JsValue] =
    action.async(parse.json) { request =>
      ...
      reqJson
        .validate[UpdateEntityJson].fold(
          errors =>
            Future.successful(
              BadRequest(Json.obj("status" -> "error", "message" -> s"invalid json values: $errors"))
            ),
          updateEntity => {
