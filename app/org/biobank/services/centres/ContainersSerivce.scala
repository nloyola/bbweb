...
trait ContainersService extends BbwebService {
  ...
  def getContainers(requestUserId: UserId, centreId: CentreId, query: PagedQuery)
...
class ContainersServiceImpl @Inject() (
  ...
  def processCommand(cmd: ContainerCommand): Future[ServiceValidation[ContainerDto]] = {
    val validCentreId = cmd match {
        case c: AddRootContainerCommand => for {
    ...
    validCentreId.fold(
      err => Future.successful((err.failure[ContainerDto])),
      centreId => whenPermittedAndIsMemberAsync(requestUserId, permission, None, Some(centreId)) { () =>
  ...
  private def filterContainersInternal(
    ...
    for {
      ...
      sortFunc <- {
        Container.sort2Compare.get(sortExpressions(0).name).
