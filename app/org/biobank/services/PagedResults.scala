...
object PagedResults {
  ...
  def createEmpty[T](page: Int, limit: Int): PagedResults[T] =
    PagedResults(items = Seq.empty[T], page = page, limit = limit, offset = 0, total = 0)

  implicit def pagedResultsWrites[T](implicit fmt: Writes[T]): Writes[PagedResults[T]] =
    new Writes[PagedResults[T]] {

      def writes(pr: PagedResults[T]): JsValue =
        Json.obj("items"    -> pr.items,
                 "page"     -> pr.page,
                 "offset"   -> pr.offset,
                 "total"    -> pr.total,
                 "limit"    -> pr.limit,
