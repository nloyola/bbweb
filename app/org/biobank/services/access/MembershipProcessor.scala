 ...
 */
class MembershipProcessor @Inject()(
  ...
  private def updateDescriptionCmdToEvent(
      cmd:        MembershipUpdateDescriptionCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] =
    membership.withDescription(cmd.description).map { _ =>
      ...
      MembershipEvent(cmd.sessionUserId).update(_.time                                   := timeStr,
                                                _.id                                     := membership.id.id,
                                                _.descriptionUpdated.version             := cmd.expectedVersion,
                                                _.descriptionUpdated.optionalDescription := cmd.description)
    }

  private def addUserCmdToEvent(
      cmd:        MembershipAddUserCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] = {
  ...
  private def updateStudyDataCmdToEvent(
    ...
    if (cmd.allStudies && cmd.studyIds.nonEmpty) {
      ...
      val subEvent = MembershipEvent
        .StudyDataUpdated()
        .update(_.version := cmd.expectedVersion, _.allStudies := cmd.allStudies, _.studyIds := cmd.studyIds)

      MembershipEvent(cmd.sessionUserId)
        .update(_.time             := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                _.id               := membership.id.id,
                _.studyDataUpdated := subEvent).successNel[String]
    }

  private def allStudiesCmdToEvent(
      cmd:        MembershipAllStudiesCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] =
    MembershipEvent(cmd.sessionUserId)
      .update(_.time               := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
              _.id                 := membership.id.id,
              _.allStudies.version := cmd.expectedVersion).successNel[String]

  // study IDs were already validated in service
  private def addStudyCmdToEvent(
      cmd:        MembershipAddStudyCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] = {
    ...
    if (membership.studyData.ids.exists(_ == studyId)) {
      ...
      MembershipEvent(cmd.sessionUserId)
        .update(_.time               := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                ...
                _.studyAdded.id      := cmd.studyId).successNel[String]
    }
  }

  // centre IDs were already validated in service
  private def updateCentreDataCmdToEvent(
      cmd:        MembershipUpdateCentreDataCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] =
    if (cmd.allCentres && cmd.centreIds.nonEmpty) {
      ...
      val subEvent = MembershipEvent
        .CentreDataUpdated().update(_.version    := cmd.expectedVersion,
      ...
      MembershipEvent(cmd.sessionUserId)
        .update(_.time              := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                _.id                := membership.id.id,
                _.centreDataUpdated := subEvent).successNel[String]
    }

  private def allCentresCmdToEvent(
      cmd:        MembershipAllCentresCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] =
    MembershipEvent(cmd.sessionUserId)
      .update(_.time               := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
              _.id                 := membership.id.id,
              _.allCentres.version := cmd.expectedVersion).successNel[String]

  private def addCentreCmdToEvent(
      cmd:        MembershipAddCentreCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] = {
    ...
    if (membership.centreData.ids.exists(_ == centreId)) {
      ...
      MembershipEvent(cmd.sessionUserId)
        .update(_.time                := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                ...
                _.centreAdded.id      := cmd.centreId).successNel[String]
    }
  }

  private def removeUserCmdToEvent(
      cmd:        MembershipRemoveUserCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] = {
    val userId = UserId(cmd.userId)
    if (!membership.userIds.exists(_ == userId)) {
      ...
      MembershipEvent(cmd.sessionUserId)
        .update(_.time                := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                ...
                _.userRemoved.id      := cmd.userId).successNel[String]
    }
  }

  private def removeStudyCmdToEvent(
      cmd:        MembershipRemoveStudyCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] = {
    val studyId = StudyId(cmd.studyId)
    if (membership.studyData.allEntities) {
      ...
      MembershipEvent(cmd.sessionUserId)
        .update(_.time                 := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                ...
                _.studyRemoved.id      := cmd.studyId).successNel[String]
    }
  }

  private def removeCentreCmdToEvent(
      cmd:        MembershipRemoveCentreCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] = {
    val centreId = CentreId(cmd.centreId)
    if (membership.centreData.allEntities) {
      ...
      MembershipEvent(cmd.sessionUserId)
        .update(_.time                  := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                ...
                _.centreRemoved.id      := cmd.centreId).successNel[String]
    }
  }

  private def removeMembershipCmdToEvent(
      cmd:        RemoveMembershipCmd,
      membership: Membership
    ): ServiceValidation[MembershipEvent] =
    MembershipEvent(cmd.sessionUserId)
      .update(_.time            := OffsetDateTime.now.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
  ...
  private def applyNameUpdatedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isNameUpdated, event.getNameUpdated.getVersion) {
      (membership, _, time) =>
  ...
  private def applyDescriptionUpdatedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event,
                                     ...
                                     event.getDescriptionUpdated.getVersion) { (membership, _, time) =>
  ...
  private def applyUserAddedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isUserAdded, event.getUserAdded.getVersion) {
      (membership, _, time) =>
        // remove this user from other memberships
        val userIdToAdd = UserId(event.getUserAdded.getId)
        membershipRepository.getValues
          .filter { membership =>
            membership.userIds.find(_ == userIdToAdd).isDefined
          }
          .foreach { membership =>
  ...
  private def applyStudyDataUpdateEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event,
                                     ...
                                     event.getStudyDataUpdated.getVersion) { (membership, _, time) =>
      val subEvent = event.getStudyDataUpdated
      val studyData =
        MembershipEntitySet(subEvent.getAllStudies, subEvent.studyIds.map(id => StudyId(id)).toSet)
      val updated =
        membership.copy(version = membership.version + 1, studyData = studyData, timeModified = Some(time))
      membershipRepository.put(updated)
      ().successNel[String]
    }

  private def applyAllStudiesEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isAllStudies, event.getAllStudies.getVersion) {
      (membership, _, time) =>
  ...
  private def applyStudyAddedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isStudyAdded, event.getStudyAdded.getVersion) {
      (membership, _, time) =>
  ...
  private def applyCentreDataUpdateEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event,
                                     ...
                                     event.getCentreDataUpdated.getVersion) { (membership, _, time) =>
      val subEvent = event.getCentreDataUpdated
      val centreData =
        MembershipEntitySet(subEvent.getAllCentres, subEvent.centreIds.map(id => CentreId(id)).toSet)
      val updated =
        membership.copy(version = membership.version + 1, centreData = centreData, timeModified = Some(time))
      membershipRepository.put(updated)
      ().successNel[String]
    }

  private def applyAllCentresEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isAllCentres, event.getAllCentres.getVersion) {
      (membership, _, time) =>
  ...
  private def applyCentreAddedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isCentreAdded, event.getCentreAdded.getVersion) {
      (membership, _, time) =>
        val updated = membership
  ...
  private def applyUserRemovedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isUserRemoved, event.getUserRemoved.getVersion) {
      (membership, _, time) =>
        val updated = membership
  ...
  private def applyStudyRemovedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isStudyRemoved, event.getStudyRemoved.getVersion) {
      (membership, _, time) =>
        val updated = membership
  ...
  private def applyCentreRemovedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event,
                                     ...
                                     event.getCentreRemoved.getVersion) { (membership, _, time) =>
      val updated = membership
  ...
  private def applyMembershipRemovedEvent(event: MembershipEvent): Unit =
    onValidMembershipEventAndVersion(event, event.eventType.isRemoved, event.getRemoved.getVersion) {
      (membership, _, time) =>
  ...
  private def processUpdateMembershipCmd[T <: MembershipModifyCommand](
      ...
      applyEvent: MembershipEvent => Unit
    ): Unit = {
    val event = for {
  ...
  private def onValidMembershipEventAndVersion(
      event:        MembershipEvent,
      eventType:    Boolean,
      eventVersion: Long
    )(applyEvent:   (Membership, MembershipEvent, OffsetDateTime) => ServiceValidation[Unit]
    ): Unit =
    if (!eventType) {
      log.error(s"invalid event type: $event")
    } else {
      membershipRepository
        .getByKey(MembershipId(event.id)).fold(
          err => log.error(s"membership from event does not exist: $err"),
          membership => {
            if (membership.version != eventVersion) {
              log.error(
                s"event version check failed: membership version: ${membership.version}, event: $event"
              )
            } else {
