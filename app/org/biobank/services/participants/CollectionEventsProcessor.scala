...
class CollectionEventsProcessor @Inject()(
  ...
  val receiveRecover: Receive = {
    case event: CollectionEventEvent =>
  ...
  private def addCmdToEvent(cmd: AddCollectionEventCmd): ServiceValidation[CollectionEventEvent] = {
    ...
    for {
      ...
      validAnnotations     <- Annotation.validateAnnotations(collectionEventType.annotationTypes, cmd.annotations)
  ...
  @silent private def updateTimeCompletedCmdToEvent(
    ...
    cevent.withTimeCompleted(cmd.timeCompleted).map { updatedCevent =>
      CollectionEventEvent(updatedCevent.id.id).update(_.participantId         := participant.id.id,
                                                       _.collectionEventTypeId := updatedCevent.collectionEventTypeId.id,
                                                       _.sessionUserId         := cmd.sessionUserId,
                                                       _.time := OffsetDateTime.now
                                                         .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
                                                       _.timeCompletedUpdated.version := cmd.expectedVersion,
                                                       _.timeCompletedUpdated.timeCompleted := updatedCevent.timeCompleted
                                                         .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME))
    }

  private def updateAnnotationCmdToEvent(
      cmd:                 CollectionEventUpdateAnnotationCmd,
      participant:         Participant,
      collectionEventType: CollectionEventType,
      cevent:              CollectionEvent
    ): ServiceValidation[CollectionEventEvent] = {
  ...
  private def removeAnnotationCmdToEvent(
      cmd:                 RemoveCollectionEventAnnotationCmd,
      participant:         Participant,
      collectionEventType: CollectionEventType,
      cevent:              CollectionEvent
    ): ServiceValidation[CollectionEventEvent] =
  ...
  @silent private def removeCmdToEvent(
    ...
    CollectionEventEvent(cevent.id.id)
  ...
  private def processUpdateCmd[T <: CollectionEventModifyCommand](
      cmd: T,
      validation: (T, Participant, CollectionEventType,
          CollectionEvent) => ServiceValidation[CollectionEventEvent],
      applyEvent: CollectionEventEvent => Unit
    ): Unit = {
    val event = for {
  ...
  private def applyAddedEvent(event: CollectionEventEvent): Unit =
    if (!event.eventType.isAdded) {
      ...
      val v = CollectionEvent.create(id = CollectionEventId(event.id),
                                     collectionEventTypeId =
                                       CollectionEventTypeId(event.getCollectionEventTypeId),
  ...
  private def onValidEventAndVersion(
    ...
    if (!eventType) {
      ...
      collectionEventRepository
        .getByKey(CollectionEventId(event.id)).fold(
  ...
  private def applyTimeCompletedUpdatedEvent(event: CollectionEventEvent): Unit =
    onValidEventAndVersion(event,
      ...
      val timeCompleted = OffsetDateTime.parse(event.getTimeCompletedUpdated.getTimeCompleted)
  ...
  private def applyAnnotationUpdatedEvent(event: CollectionEventEvent): Unit =
    onValidEventAndVersion(event, event.eventType.isAnnotationUpdated, event.getAnnotationUpdated.getVersion) {
      (cevent, eventTime) =>
  ...
  private def applyAnnotationRemovedEvent(event: CollectionEventEvent): Unit =
    onValidEventAndVersion(event, event.eventType.isAnnotationRemoved, event.getAnnotationRemoved.getVersion) {
      (cevent, eventTime) =>
   ...
   */
  protected def visitNumberAvailableMatcher(
      visitNumber: Int
    )(matcher:     CollectionEvent => Boolean
    ): ServiceValidation[Unit] = {
  ...
  private def visitNumberAvailable(participantId: ParticipantId, visitNumber: Int): ServiceValidation[Unit] =
    visitNumberAvailableMatcher(visitNumber) { item =>
      (item.participantId == participantId) && (item.visitNumber == visitNumber)
    }

  @SuppressWarnings(Array("org.wartremover.warts.Overloading"))
  private def visitNumberAvailable(
