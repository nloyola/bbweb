 ...
 *
 */
final case class CollectionEventType(
    studyId:             StudyId,
  ...
  def withRecurring(recurring: Boolean): DomainValidation[CollectionEventType] =
    copy(recurring = recurring, version = version + 1, timeModified = Some(OffsetDateTime.now))
      .successNel[String]

  def withAnnotationType(annotationType: AnnotationType): DomainValidation[CollectionEventType] =
