...
trait StudiesServiceFixtures extends ProcessorTestFixture with UserServiceFixtures {
  ...
  class UsersWithStudyAccessFixture {
    ...
    val allStudiesMembership = factory.createMembership
      .copy(userIds = Set(allStudiesAdminUser.id), studyData = MembershipEntitySet(true, Set.empty[StudyId]))
    ...
    def usersCannotReadTable() =
      Table(("users without read access", "label"),
    ...
    Set(study,
        ...
        studyOnlyMembership,
        noStudiesMembership).foreach(addToRepository)
