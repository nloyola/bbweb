 ...
 */
class StudiesServiceSpec extends ProcessorTestFixture with StudiesServiceFixtures {
  ...
  class StudyOfAllStatesFixure extends UsersWithStudyAccessFixture {
    ...
    addToRepository(
      studyOnlyMembership.copy(
        studyData =
          studyOnlyMembership.studyData.copy(ids = Set(disabledStudy.id, enabledStudy.id, retiredStudy.id))
      )
    )
  ...
  private def updateCommandsTable(sessionUserId: UserId, study: Study, annotationType: AnnotationType) =
    Table("study update commands",
          UpdateStudyNameCmd(sessionUserId                         = Some(sessionUserId.id),
                             id                                    = study.id.id,
                             expectedVersion                       = study.version,
                             name                                  = nameGenerator.next[String]),
          UpdateStudyDescriptionCmd(sessionUserId                  = Some(sessionUserId.id),
                                    id                             = study.id.id,
                                    expectedVersion                = study.version,
                                    description                    = Some(nameGenerator.next[String])),
          StudyAddParticipantAnnotationTypeCmd(sessionUserId       = Some(sessionUserId.id),
                                               id                  = study.id.id,
                                               expectedVersion     = study.version,
          ...
          StudyUpdateParticipantAnnotationTypeCmd(sessionUserId    = Some(sessionUserId.id),
                                                  id               = study.id.id,
                                                  expectedVersion  = study.version,
                                                  ...
                                                  options          = annotationType.options,
                                                  required         = annotationType.required),
          UpdateStudyRemoveAnnotationTypeCmd(sessionUserId         = Some(sessionUserId.id),
                                             id                    = study.id.id,
                                             expectedVersion       = study.version,
                                             annotationTypeId      = annotationType.id.id))
  ...
  private def stateChangeCommandsTable(
    ...
    Table("user sate change commands",
          EnableStudyCmd(sessionUserId     = Some(sessionUserId.id),
                         id                = disabledStudy.id.id,
                         expectedVersion   = disabledStudy.version),
          DisableStudyCmd(sessionUserId    = Some(sessionUserId.id),
                          id               = enabledStudy.id.id,
                          expectedVersion  = enabledStudy.version),
          RetireStudyCmd(sessionUserId     = Some(sessionUserId.id),
                         id                = disabledStudy.id.id,
                         expectedVersion   = disabledStudy.version),
          UnretireStudyCmd(sessionUserId   = Some(sessionUserId.id),
