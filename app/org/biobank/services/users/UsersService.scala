...
class UsersServiceImpl @javax.inject.Inject()(
  ...
  private def filterUsers(filter: FilterString, sort: SortString): ServiceValidation[Seq[UserDto]] = {
    ...
    val sortStr =
      if (sort.expression.isEmpty) new SortString("email")
      else sort
    for {
  ...
  def getCountsByStatus(requestUserId: UserId): ServiceValidation[UserCountsByStatus] =
    whenPermitted(requestUserId, PermissionId.UserRead) { () =>
      ...
      UserCountsByStatus(total           = users.size.toLong,
  ...
  def processCommand(cmd: UserCommand): FutureValidation[UserDto] = {
    ...
    cmd match {
      ...
      case c: UserModifyCommand =>
        val v = accessService
          .hasPermission(UserId(c.sessionUserId), PermissionId.UserUpdate)
          .map { permission =>
  ...
  private def processUserAccessCommand(cmd: UserAccessCommand): FutureValidation[UserDto] = {
    ...
    cmd match {
      ...
      case c: UpdateUserAddMembershipCmd =>
        val v = for {
          user       <- checkUserAndVersion(UserId(c.id), c.expectedVersion)
          membership <- accessService.getMembership(UserId(c.sessionUserId), MembershipId(c.membershipId))
