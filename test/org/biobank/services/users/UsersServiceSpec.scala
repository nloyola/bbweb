...
class UsersServiceSpec
  ...
  def commandsTable(sessionUserId: UserId, user: User, userPlainPassword: String) =
    Table("user update commands",
          UpdateUserNameCmd(sessionUserId        = sessionUserId.id,
                            id                   = user.id.id,
                            expectedVersion      = user.version,
                            name                 = faker.Name.name),
          UpdateUserEmailCmd(sessionUserId       = sessionUserId.id,
                             id                  = user.id.id,
                             expectedVersion     = user.version,
                             email               = nameGenerator.nextEmail[User]),
          UpdateUserPasswordCmd(sessionUserId    = sessionUserId.id,
                                id               = user.id.id,
                                expectedVersion  = user.version,
                                currentPassword  = userPlainPassword,
                                newPassword      = nameGenerator.next[String]),
          UpdateUserAvatarUrlCmd(sessionUserId   = sessionUserId.id,
                                 id              = user.id.id,
                                 expectedVersion = user.version,
                                 avatarUrl       = Some(nameGenerator.nextUrl[String])))

  def stateChangeCommandsTable(
    ...
    Table("user sate change commands",
          ActivateUserCmd(sessionUserId   = sessionUserId.id,
                          id              = registeredUser.id.id,
                          expectedVersion = registeredUser.version),
          LockUserCmd(sessionUserId       = sessionUserId.id,
                      id                  = activeUser.id.id,
                      expectedVersion     = activeUser.version),
          UnlockUserCmd(sessionUserId     = sessionUserId.id,
                        id                = lockedUser.id.id,
